...
function PhanxDebuffFrame:UpdateLayout()
	...
	-- if db.debuffPoint and db.debuffX and db.debuffY then
		-- self:SetPoint(db.debuffPoint, SUFUnitplayer, db.debuffX, db.debuffY + 0.5)
	-- else
		-- self:SetPoint("BOTTOMRIGHT", SUFUnitplayer, -70 - floor(Minimap:GetWidth() + 0.5), floor(SUFUnitplayer:GetHeight() + 0.5) - floor(Minimap:GetHeight() + 0.5) - 62)
	-- end
	--if db.debuffPoint and db.debuffX and db.debuffY then
		--self:SetPoint("TOPLEFT", SUFUnitplayer, -105, 60)
	--else
		--self:SetPoint("BOTTOMRIGHT", SUFUnitplayer,  0, 0)
	--end

	local currentSpec = GetSpecialization()
	local currentSpecName = select(2, GetSpecializationInfo(currentSpec))
	if (currentSpecName == "戒律") or (currentSpecName == "神圣") then
		self:SetPoint("CENTER", SUFUnitplayer, -410, 110)
	else
		self:SetPoint("CENTER", SUFUnitplayer, -8, 70)
	end
