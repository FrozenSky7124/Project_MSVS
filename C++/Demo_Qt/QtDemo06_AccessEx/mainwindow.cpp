    ...
    : QWidget(parent)
{
    ...
    //剑侠情缘网络版三（重制版）
    openDB_JX3Ex_Btn = new QPushButton(tr("连接数据库"), this);
    createTable_JX3Ex_Btn = new QPushButton(tr("创建数据表"), this);
    importFile_JX3Ex_Btn1 = new QPushButton(tr("问题导入"), this);
    importFile_JX3Ex_Btn2 = new QPushButton(tr("拼音导入"), this);
    updateDB_JX3Ex_Btn = new QPushButton(tr("更新"), this);
    selectTable_JX3Ex_Btn = new QPushButton(tr("显示数据"), this);
    //主窗口
    ...
    //剑侠情缘网络版三（重制版） 2017-06-02
    layout->addWidget(openDB_JX3Ex_Btn, 2, 0, 1, 1);
    layout->addWidget(createTable_JX3Ex_Btn, 2, 1, 1, 1);
    layout->addWidget(importFile_JX3Ex_Btn1, 2, 2, 1, 1);
    layout->addWidget(importFile_JX3Ex_Btn2, 2, 3, 1, 1);
    layout->addWidget(updateDB_JX3Ex_Btn, 2, 4, 1, 1);
    layout->addWidget(selectTable_JX3Ex_Btn, 2, 5, 1, 1);

    layout->addWidget(mainTextEdit,3,0,1,6);
    ...
    //剑侠情缘网络版三（重制版） 2017-06-02
    connect(openDB_JX3Ex_Btn, SIGNAL(clicked(bool)), this, SLOT(openDB_JX3Ex()));
    connect(createTable_JX3Ex_Btn, SIGNAL(clicked(bool)), this, SLOT(createTable_JX3Ex()));
    connect(importFile_JX3Ex_Btn1, SIGNAL(clicked(bool)), this, SLOT(importFile_JX3Ex1()));
    connect(importFile_JX3Ex_Btn2, SIGNAL(clicked(bool)), this, SLOT(importFile_JX3Ex2()));
    connect(updateDB_JX3Ex_Btn, SIGNAL(clicked(bool)), this, SLOT(updateDB_JX3Ex()));
    connect(selectTable_JX3Ex_Btn, SIGNAL(clicked(bool)), this, SLOT(selectTable_JX3Ex()));
}
...
void Widget::updateDB()
{
    ...
    // 2016-07-07
    //70817 ~ 71096 与 78817  ~  79096 ***** 13537=71073=79073 【13281=78817】***** 7月7日更新切换
    //136633~136772 与 144633 ~ 144772 ***** 13605=136677=144677 【13561=144633】***** 7月7日更新切换
    updateSql_TY(78817, 79096, 13281);
    updateSql_TY(144633, 144772, 13561);
}
...
//剑侠情缘网络版三（重制版）
bool Widget::openDB_JX3Ex()
{
    //添加数据库
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    //设置数据库名称
    db.setDatabaseName("dbQA_JX3Ex.db");
    //打开数据库
    if( !db.open() )
    {
        QMessageBox::information(this, "Error", "数据库连接失败！");
...
bool Widget::createTable_JX3Ex()
{
    QSqlQuery query;
    if (true == query.exec("CREATE TABLE qatable(id int NOT NULL, pyquestion varchar, question varchar, answer varchar, PRIMARY KEY (id))"))
    {
        QMessageBox::information(this, "Create", "数据表创建完成！");
        return true;
    }
    ...
    {
        QMessageBox::information(this, "Create", "数据表创建失败！");
        return false;
    }
}

void Widget::insertToDB_JX3Ex(int id, QString pyquestion, QString question, QString answer)
{
    QSqlQuery query;
    QString sqlstr;
    sqlstr.sprintf("INSERT INTO qatable VALUES('%d', '%s', '%s', '%s')",
                   id,
                   pyquestion.toStdString().data(),
                   question.toStdString().data(),
                   answer.toStdString().data());
...
void Widget::updatePyToDB_JX3Ex(int id, QString pyquestion)
{
    QSqlQuery query;
    QString sqlstr;
    sqlstr.sprintf("UPDATE qatable SET pyquestion = '%s' WHERE id = '%d'", pyquestion.toStdString().data(), id);
    query.exec(sqlstr);
}

void Widget::importFile_JX3Ex1()
{
    //获得文件路径
    QString filePath = QFileDialog::getOpenFileName(this, tr("Open DataFile"), QDir::currentPath());
    if(filePath.isEmpty())
    ...
    while(!tempStr.isEmpty())
    {
        ...
        QStringList tempList2 = question.split("单选题：");//再次分割题目
        const QString question2 = tempList2.at(1);
        mainTextEdit->append(QString(index) + " " + question2 + " " + answer);
        insertToDB_JX3Ex(index, "", question2, answer);
        tempStr = stream->readLine();
    }
    file.close();
    QMessageBox::information(this, "Import", "导入数据库完成！");
...
void Widget::importFile_JX3Ex2()
{
    ...
    while(!tempStr.isEmpty())
    {
        ...
        QStringList tempList2 = question.split("单选题：");//再次分割题目
        const QString question2 = tempList2.at(1);
        updatePyToDB_JX3Ex(index, ToChineseSpell(question2));
        tempStr = stream->readLine();
    }
    file.close();
    QMessageBox::information(this, "Import", "导入数据库完成！");
