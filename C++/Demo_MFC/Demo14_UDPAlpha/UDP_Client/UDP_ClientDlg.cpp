...
BOOL CUDP_ClientDlg::InitSocket()
{
	...
	else
	{
		...
		iResult = bind(m_socket, (SOCKADDR *)& addr_recv, sizeof(addr_recv));
		if (iResult != 0)
		{
...
void CUDP_ClientDlg::OnBnClickedButtonSend()
{
	...
	GetDlgItemText(IDC_EDIT_IP, ip_SendTo);
	m_addrSendto.sin_addr.S_un.S_addr = inet_addr(ip_SendTo);
	...
	UDP_CONFIRM confirmPackage;
	int iSenderAddrSize = sizeof(m_addr_sender);
	...
	for (int i = 0; i <= iCount; i++)
	{
		...
		// 监听确认包
		iResult = recvfrom(m_socket, (char *)&confirmPackage, sizeof(UDP_CONFIRM), 0, (SOCKADDR*)&m_addr_sender, &iSenderAddrSize);
		if ((((UDP_CONFIRM)(confirmPackage)).flag == 1) && (((UDP_CONFIRM)(confirmPackage)).iRecvNum == i))
			continue;
		else
			AfxMessageBox(_T("ConfirmPackage receive error!"));
	}
